...
try:
    ...
    print(
        "Error: weasyprint required. Install with: pip install weasyprint",
        file=sys.stderr,
    )
    sys.exit(1)
...
def chart_lighthouse_gauges(data: dict, output_dir: Path) -> str:
    ...
    for ax, (key, label) in zip(axes.flat, categories):
        ...
        ax.plot(
            theta_bg, [1] * 100, linewidth=14, color="#e2e8f0", solid_capstyle="round"
        )
        ax.plot(
            theta_fill,
            [1] * 100,
...
def chart_cwv_timeline(data: dict, output_dir: Path) -> str:
    ...
    fig, axes = plt.subplots(
        len(available), 1, figsize=(10, 3 * len(available)), sharex=True
    )
    if len(available) == 1:
        axes = [axes]
    ...
    for ax, metric_name in zip(axes, available):
        ...
        if good_t and poor_t:
            ...
            ax.axhline(
                y=good_t, color=BRAND["success"], linestyle="--", alpha=0.5, linewidth=1
            )
            ax.axhline(
                y=poor_t, color=BRAND["danger"], linestyle="--", alpha=0.5, linewidth=1
            )
...
def chart_top_queries(data: dict, output_dir: Path) -> str:
    ...
    y = range(len(labels))
    bars = ax.barh(
        y, impressions, color=BRAND["primary"], height=0.55, label="Impressions"
    )
...
def _build_title_page(
    domain, report_title, subtitle, score=None, score_label=None, meta_items=None
):
...
def _build_executive_summary(domain, timestamp, data, report_type):
    ...
    # Context paragraph
    lines.append(
        f"  <p>This report presents a comprehensive Google SEO analysis of "
    ...
    if failed_audits:
        ...
        for a in top_fail:
            issues.append(
                f'<strong>{a.get("title", "Unknown")}</strong> '
    ...
    if qw:
        wins.append(
            f"{len(qw)} search queries at positions 4-10 with high impressions "
...
def _build_cwv_section(
    psi_data, crux_data, chart_paths, history_data=None, section_num=2
):
    ...
    if perf is not None:
        ...
        lines.append(
            f'    <div class="section-score" style="color: {color};">{perf}/100</div>'
        )
        ...
        for key, label in [
            ...
            if s is not None:
                cls = (
                    "status-pass"
                    if s >= 90
                    else ("status-warn" if s >= 50 else "status-fail")
                )
                rating = "Good" if s >= 90 else ("Needs Work" if s >= 50 else "Poor")
                lines.append(
                    f'      <tr><td>{label}</td><td class="{cls}">{s}</td><td>{rating}</td></tr>'
                )
        lines.append("    </tbody>")
        lines.append("  </table>")
        lines.append("")
    ...
    if lab:
        ...
        for k, v in lab.items():
            ...
            cls = (
                "status-pass"
                if score_val and score_val >= 0.9
                else (
                    "status-warn" if score_val and score_val >= 0.5 else "status-fail"
                )
            )
            label = metric_labels.get(k, k.replace("-", " ").title())
            threshold = thresholds.get(k, "\u2014")
            lines.append(
                f'      <tr><td>{label}</td><td>{v.get("display", "")}</td>'
    ...
    if crux_metrics:
        lines.append(
            f"  <h3>{section_num}.3 CrUX Field Data (28-day Rolling Average)</h3>"
        )
        lines.append("")
        ...
        lines.append("    <thead>")
        lines.append(
            "      <tr><th>Metric</th><th>p75</th><th>Rating</th>"
        ...
        for name, m in crux_metrics.items():
            ...
            p75 = m.get("p75", "?")
            display_val = (
                f"{p75:.3f}" if name == "cumulative_layout_shift" else f"{p75}{unit}"
            )
            cls = _rating_css_class(rating)
            lines.append(
                f'      <tr><td>{m.get("label", name)}</td><td>{display_val}</td>'
            )
            lines.append(f'      <td class="{cls}">{rating.upper()}</td>')
            lines.append(
                f'      <td>{dist.get("good", "N/A")}%</td>'
        ...
        if cp:
            lines.append(
                f'  <p class="data-freshness">Collection period: {cp.get("first", "?")} '
        ...
        lines.append(
            f'  <div class="highlight"><strong>CrUX Field Data:</strong> '
            f'{crux["error"]}</div>'
        )
        lines.append("")
    ...
    if history_data and not history_data.get("error"):
        ...
        if trends:
            ...
            lines.append("    <thead>")
            lines.append(
                "      <tr><th>Metric</th><th>Direction</th><th>Change</th>"
            ...
            for name, t in trends.items():
                ...
                lines.append(
                    f'      <tr><td>{t.get("label", name)}</td>'
                    f'<td class="{cls}">{direction.upper()}</td>'
                )
                lines.append(
                    f'      <td>{t.get("change_pct", 0):+.1f}%</td>'
    ...
    if seo_audits:
        ...
        if seo_failed:
            ...
            lines.append(
                f'  <div class="success-box"><strong>SEO:</strong> '
...
def _build_gsc_section(gsc_data, chart_paths, section_num=3, fig_start=1):
    ...
    if totals:
        ...
        lines.append(
            f'  <p>Period: {dr.get("start", "?")} to {dr.get("end", "?")} '
            f'| Property: {domain}</p>'
        )
        queries_count = gsc_data.get("row_count", 0)
        impr_total = totals.get("impressions", 0)
        lines.append(
            f"  <p><strong>{domain}</strong> appeared in <strong>{queries_count}</strong> unique search queries "
        ...
        lines.append(
            f'    <div class="col">{_metric_card(clicks_val, "Total Clicks", BRAND["primary"])}</div>'
        )
        lines.append(
            f'    <div class="col">{_metric_card(impr_val, "Total Impressions", BRAND["secondary"])}</div>'
        )
        lines.append(
            f'    <div class="col">{_metric_card(ctr_val, "Average CTR", BRAND["accent"])}</div>'
        )
        lines.append(
            f'    <div class="col">{_metric_card(rows_val, "Queries Found", BRAND["dark"])}</div>'
        )
        lines.append("  </div>")
        lines.append('  <hr class="divider">')
        lines.append("")
    ...
    if rows:
        ...
        lines.append("    <thead>")
        lines.append(
            "      <tr><th>#</th><th>Query</th><th>Clicks</th>"
    ...
    if rows:
        ...
        if beyond:
            lines.append(
                f"  <p>{beyond} queries rank beyond position 10 "
    ...
    if qw:
        ...
        lines.append(
            '  <div class="highlight">These queries rank at position 4-10 with '
    ...
    lines.append(
        '  <p class="data-freshness">Search Analytics data has a 2-3 day lag. '
...
def _build_indexation_section(inspect_data, chart_paths, section_num=4, fig_start=1):
    ...
    if summary:
        ...
        if idx_path:
            ...
            lines.append(
                f'      <img src="file://{idx_path}" style="width: 70%;" alt="Index status donut chart">'
            )
            lines.append(
                f'      <div class="chart-caption">Figure {fig_n}: URL indexation status distribution from Google URL Inspection API.</div>'
            )
            lines.append("    </div>")
        ...
        lines.append('    <div class="col">')
        lines.append(
            _metric_card(summary.get("fail", 0), "Not Indexed", BRAND["danger"])
        )
        lines.append("    </div>")
        lines.append("  </div>")
        lines.append("")
        ...
        if total > 0:
            ...
            lines.append(
                f"  <p><strong>Index Rate:</strong> {rate}% of inspected URLs are indexed by Google.</p>"
            )
        if total > 0 and indexed > 0:
            ...
            cls = (
                "success-box"
                if pct >= 90
                else ("highlight" if pct >= 70 else "critical-box")
            )
            lines.append(
                f'  <div class="{cls}"><strong>Index Rate:</strong> '
    ...
    if results:
        ...
        for r in results:
            ...
            lines.append(
                f'      <tr><td style="word-break:break-all;font-size:8pt;">'
                f"{url_display}</td>"
            )
            lines.append(
    ...
    lines.append(
        '  <p class="data-freshness">URL Inspection API: '
...
def _build_recommendations(data, section_num=5):
    ...
    lines.append("")
    lines.append(
        "  <p>Prioritized action items based on the data collected. "
    ...
    for a in seo_failed[:2]:
        critical_items.append(
            (
                f"Fix SEO Issue: {a.get('title', 'Unknown')}",
                "Low (30 min)",
    ...
    if not_indexed:
        critical_items.append(
            (
                f"Resolve {not_indexed} Non-Indexed URL(s)",
                "Medium (2-4 hrs)",
    ...
    if critical_items:
        lines.append(
            '  <h3><span class="priority-tag priority-critical">CRITICAL</span> '
            "Fix Immediately</h3>"
        )
    ...
    if qw:
        high_items.append(
            (
                f"Optimize {len(qw)} Quick-Win Queries",
                "Medium (2-4 hrs)",
    ...
    if high_items:
        lines.append(
            '  <h3><span class="priority-tag priority-high">HIGH</span> '
    ...
    if a11y:
        medium_items.append(
            (
                f"Fix {len(a11y)} Accessibility Issue(s)",
                "Low (30 min)",
    ...
    if medium_items:
        lines.append(
            '  <h3><span class="priority-tag priority-medium">MEDIUM</span> '
    ...
    if item_num == 0:
        lines.append(
            '  <div class="success-box"><strong>No critical issues detected.</strong> '
    ...
    if perf is not None and perf < 50:
        lines.append(
            "      <li>Optimize Largest Contentful Paint and Total Blocking Time</li>"
        )
    if not_indexed:
        lines.append(f"      <li>Resolve {not_indexed} non-indexed URL(s)</li>")
    if opps:
        lines.append(
            f"      <li>Implement {len(opps)} performance optimization(s)</li>"
        )
    if (perf is None or perf >= 50) and not not_indexed and not opps:
        lines.append(
            "      <li>Maintain current performance levels and monitor trends</li>"
        )
    lines.append("    </ul>")
    lines.append("  </div>")
    lines.append('  <div class="roadmap-phase">')
    ...
    if qw:
        lines.append(
            f"      <li>Optimize {len(qw)} quick-win queries for top-3 rankings</li>"
        )
    lines.append(
        "      <li>Review and improve content depth for underperforming pages</li>"
    )
...
def generate_report(report_type, data, domain, output_dir, output_format="pdf"):
    ...
    if report_type == "cwv-audit":
        ...
        perf_score = (
            mobile.get("lighthouse_scores", {}).get("performance")
            if isinstance(mobile, dict)
            else None
        )
    ...
    # ── INDEXATION report ────────────────────────────────────────────────────
        ...
        idx_html, fig_num = _build_indexation_section(
            inspect, chart_paths, section_num=2
        )
    ...
    for i, sec in enumerate(sections[1:], 1):
        ...
        if len(text_only) < 50:
            review["issues"].append(
                f"Section {i} has very little text content ({len(text_only)} chars)"
            )
...
def generate_xlsx(data, domain, report_type, output_dir):
    ...
    try:
        ...
        print(
            "Warning: openpyxl not installed. Skipping xlsx. Install: pip install openpyxl",
            file=sys.stderr,
        )
        return None
    ...
    cream_fill = PatternFill(
        start_color="FAF9F7", end_color="FAF9F7", fill_type="solid"
    )
            ...
            for cat in ["performance", "accessibility", "best_practices", "seo"]:
                ...
                if val is not None:
                    ...
                    ws.append(
                        [
                            cat.replace("_", " ").title(),
                            int(val * 100) if val <= 1 else val,
                        ]
                    )
                    ws.cell(row=row_num, column=2).fill = _severity_fill(
                        "pass"
                        if (val * 100 if val <= 1 else val) >= 90
                        else "warning"
                        if (val * 100 if val <= 1 else val) >= 50
                        else "fail"
                    )
            ws.append([])
            ...
            if isinstance(row_data, dict):
                ...
                ws2.append(
                    [
                        ...
                        f"{row_data.get('ctr', 0):.2%}"
                        if isinstance(row_data.get("ctr"), (int, float))
                        else str(row_data.get("ctr", "")),
                        round(row_data.get("position", 0), 1)
                        if isinstance(row_data.get("position"), (int, float))
                        else row_data.get("position", ""),
                    ]
                )
...
def main():
    ...
    parser.add_argument(
        "--data", "-d", help="Path to JSON data file (or pipe via stdin)"
    )
    parser.add_argument(
        "--domain", required=True, help="Domain name for the report header"
    )
    parser.add_argument(
