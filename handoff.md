...
**Found, deliberately NOT applied — open for a future session:**
1. **`expand_query_with_ontology` (rag/nodes/retrieval.py:1128) runs serially before the retrieval fan-out**, up to 3s of pure latency on every non-fast RELATIONAL/FACTUAL/QUERY request when `KNOWLEDGE_GRAPH_QUERY_ENABLED=true`. Fix would fire it as a concurrent task alongside the primary embed/BM25 dispatch (same "fire concurrently, consume later" pattern already used for `_llm_retrieval_expansions` in the same file) — medium risk, needs live profiling to size the actual win before touching.
2. **`query_neo4j_subgraph` (RELATIONAL intent, retrieval.py:905) is awaited sequentially after the LightRAG-merge branch** rather than joining the primary `asyncio.gather` at line 829 — same class of fix as #1, same "needs profiling first" caveat.
