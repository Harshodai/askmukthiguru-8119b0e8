...
- **Phase B (evals): scaffold real and running, far from complete.** `evals/scenarios/` has 14 hand-authored multi-turn scenarios (not PLAN.md's 60+), English only. `evals/rubrics/safety_rubric.yaml` defines 7 scoring dimensions. `evals/run_safety_scenarios.py` actually executes tier-3 scenarios against the real classifier (no LLM needed — crisis preemption is deterministic), wired into CI. `evals/bakeoff/questions.yaml` has 15 of a planned 50 NotebookLM comparison questions, no comparison has been run. **B2 (grounding evals) started same day**: `evals/grounding/verify_quote.py` checks verbatim-quote and teacher-attribution claims against `transcripts/*.md` (763 real transcript files — not `memory/okf/*.md`, which turned out to be LLM-synthesized summary, not verbatim source, checked directly before building against it), plus a `precision_recall_f1()` scorer; wired into CI too. **B3 (tone/impersonation) started same day**: `backend/rag/nodes/verification.py`'s `check_constitutional_compliance()` (already live in the `reflect_on_answer` LangGraph node) already had `_FOUNDER_IMPERSONATION_RE`; added `_SPIRITUAL_AUTHORITY_CLAIM_RE` to close brief N2's separate "never grant diksha/absolution/blessing/spiritual authority" requirement, which the impersonation regex didn't cover (identity claims vs. authority grants are different failure modes). 6 true-positive + 5 false-positive regression tests in `backend/tests/test_nodes.py`, all passing through the real import path; 397-test regression sweep and lint both clean. Deliberately not duplicated into a parallel `evals/tone/` scanner — documented in `evals/README.md` instead. B3's other half (blind human/faculty tone rating) needs a live backend and a rater, not started. Tiers 0-2 of B1 and all of B2's "real citation" checking still need live generation, which needs the backend to actually be running (see §5).
- **Phase G (18+ gate): started.** `src/pages/AuthPage.tsx` required checkbox ("I confirm I am 18 years of age or older") before the email/password submit button or Google/Apple OAuth buttons become clickable — gated both via disabled UI state and independently in the submit handler. Email-path confirmations recorded in Supabase `user_metadata`; OAuth-path has no equivalent pre-redirect metadata hook, so it's UI-gated only (see `lessons.md` `L-AGE-GATE-1`). Verified live in-browser (checkbox unchecked → Google button excluded from the accessibility tree; checked → reappears, submit enables) plus 3 new + 2 updated Vitest tests; full frontend suite 609/609 pass, lint/typecheck clean. **This session's original brief text for Phases C–I was lost to a context compaction mid-session** — rather than rebuild those phases from a paraphrase, only G was pursued (independently confirmed missing via grep, safely scoped without the lost text). C/D/E/F/H/I remain untouched.
