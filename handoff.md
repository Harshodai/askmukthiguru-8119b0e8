...
## 9. Third crash (real OOM, not a code bug) + raised memory ceiling + committed everything

Re-ran the canary after RULE 42 was deployed — crashed a third time. This one genuinely was memory exhaustion: `RuntimeError: could not create a primitive` (PyTorch/oneDNN allocation failure) inside LettuceDetect's embedding scoring, then `memory allocation of 8388608 bytes failed` → `Fatal Python error: Aborted`. The canary's Auto-Flush step wipes the semantic cache first, so all 26 questions hit full cold-path CPU inference (BGE-M3 + reranker + LettuceDetect) back to back — a real, not-yet-optimized memory-intensity issue, not a logic bug. With user sign-off, raised `PYTHON_MEMORY_LIMIT_MB` 5632→12288 (Railway's real ceiling is 24GB, confirmed earlier — 12GB leaves 12GB headroom).

**Important Railway behavior discovered**: a variable-set redeploy rebuilds from the **connected GitHub repo's last commit**, not from a prior `railway up` tarball. Changing the memory variable silently reverted the running code to git HEAD, discarding both RULE 41 and RULE 42 until a fresh `railway up` was pushed on top. Also hit a stuck "Initializing" deploy state for ~18 minutes after three rapid overlapping deploy triggers (var-change + two `railway up`s) — resolved on its own, no clear single cause identified, `railway up` remains the reliable unstick lever per RULE 39 but this took longer than any prior instance this session.

**Also discovered**: this sandbox's `curl` cannot reliably reach `askmukthiguru-8119b0e8-production.up.railway.app` right now (connects TLS, then hangs indefinitely) while the Claude Browser tool reaches it fine — a local/sandbox networking quirk, not a backend problem. Use the browser tool for health verification going forward, not `curl`, if this recurs.

**Resolution**: at user's direction, committed and pushed the verified, tested fixes to `main` (commit `a6badadd`: RULE 41 + RULE 42 + question bank expansion + dashboard/benchmark-suite fixes + this file + lessons.md — 8 files, deliberately excluding the still-stashed ContextBudgetManager/Qdrant-API-key group and pre-existing unrelated root-level files), then `railway up` from that clean committed state. Verified healthy via browser tool: `ready:true`, all critical services ok. Neo4j shows `ok:false, latency_ms:60028` — separate, pre-existing, non-critical issue (its own deploy failed independently around the same time per `railway status`); not investigated further, not blocking chat.

**Remaining**: `git stash list` still has one entry (ContextBudgetManager + Qdrant API key + ingest pipeline changes) — deliberately not committed, not deployed, per earlier sections. Re-running the expanded question-bank benchmark against prod is still pending — do it cautiously (low concurrency, don't flush caches first this time to avoid forcing worst-case cold-path memory pressure) now that the memory ceiling has real headroom.
