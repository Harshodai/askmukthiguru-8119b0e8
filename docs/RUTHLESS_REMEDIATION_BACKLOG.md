...
Implement a cheap retrieval planner before expensive retrieval operations.
...
`intent -> Qdrant + Neo4j/LightRAG in parallel -> bounded graph expansion -> rerank/evidence gate -> answer`
...
## 4. P1 — Neo4j/LightRAG must earn its latency

Do not maximize graph usage for its own sake.
