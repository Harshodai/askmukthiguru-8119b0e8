...
- Neo4j + LightRAG for graph/entity/relationship retrieval.
...
The graph layer is used for relationship/entity context rather than as a replacement for Qdrant. It is an enrichment/retrieval path inside the broader RAG system and can degrade when unavailable.
...
**Yes, they are genuinely wired into the architecture and not merely present as dead dependencies.** Current source has a LightRAG service, Neo4j driver access, graph retrieval nodes, graph state and admin RAG-flow visibility. The developer guide also describes Qdrant + Neo4j as part of the active retrieval topology.
...
9. Treat LightRAG workspace/collection naming as a migration-sensitive contract.
