...
The Neo4j, Redis, Qdrant, and backend health-check intervals were changed from 10 seconds to 30 seconds. This reduces local probe frequency while retaining health checks. It does not change Railway billing and is not presented as a measured dollar saving. Local tracing is now opt-in because the Jaeger service is an observability profile; the default Compose serving stack sets `OTEL_ENABLED=false` unless explicitly overridden.

The Compose file now exposes overrideable Neo4j memory settings with defaults of `256M` initial heap, `512M` maximum heap, and `512M` page cache. A disposable Neo4j 5.17.0 canary using `256M` initial heap, `512M` maximum heap, `256M` page cache, no plugins, and an empty volume started successfully and displayed `491.6 MiB` memory. This is startup evidence only; no graph query or latency benchmark was run, so the setting is not yet proven safe for the existing data volume.
