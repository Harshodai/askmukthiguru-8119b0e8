...
Do not optimize for “use every subsystem.” Qdrant, Neo4j/LightRAG, reranking, CRAG, reflection and verification should each earn their latency through measurable quality improvement.
...
| P1 | Graph economics | Neo4j/LightRAG is useful but should not run expensively for every query | Query-adaptive graph routing |
...
Neo4j/LightRAG should be **query-adaptive**.
...
Persist and validate an index fingerprint containing at least:
...
- [ ] Build query-adaptive retrieval planner.
...
Neo4j and LightRAG should stay, but become economically accountable. Qdrant should remain the primary broad retrieval engine, while graph retrieval earns its place through relational accuracy. Reflection, CRAG and model verification should be escalations, not rituals.
