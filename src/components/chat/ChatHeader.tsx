...
export const ChatHeader = ({
  ...
  return (
    <header
      className={`relative z-20 sticky top-0 backdrop-blur-md border-b border-border/30 h-[56px] sm:h-[64px] safe-top ${
        isIncognito
          ? 'bg-amber-950/15'
          : 'bg-background/85 bg-gradient-to-r from-ojas/5 to-transparent'
      }`}
      data-testid="chat-header-simplified"
    >
      <div className="flex items-center justify-between px-2.5 sm:px-5 h-full">
        <div className="flex items-center gap-1 sm:gap-2 min-w-0">
          {onOpenMobileMenu && (
            <Button
              ...
              className="sm:hidden min-h-[44px] min-w-[44px] h-10 w-10 rounded-xl"
          ...
          <Link
            to="/"
            className="hidden sm:flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg hover:bg-muted transition-colors"
            title={t('nav.home')}
          ...
          {isIncognito ? (
            <div className="flex items-center gap-2 ml-1 min-w-0">
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-amber-600/40 bg-amber-950/20 text-amber-600 text-[11px] font-medium whitespace-nowrap">
            ...
            <span
              className="flex items-center gap-1.5 font-serif font-semibold text-foreground text-sm ml-1 select-none"
        ...
        <div className="flex items-center gap-0.5 sm:gap-1.5">
          ...
          {isIncognito && (
            <span className="text-[10px] text-amber-600/70 hidden sm:block mr-1">
              {t('chat.incognitoDescription')}
            </span>
          )}
          ...
          {hasMessages && onOpenSources && (
            <Button
              ...
              className="min-h-[44px] min-w-[44px] h-10 w-10 sm:h-8 sm:w-8 text-muted-foreground hover:text-foreground relative flex items-center justify-center rounded-xl"
              aria-label={t('chat.openSources', { count: sourcesCount })}
              title={t('chat.viewSources')}
            >
              <Library className="w-4 h-4" />
              {sourcesCount > 0 && (
                <span className="absolute top-0 right-0 inline-flex items-center justify-center min-w-[16px] h-[16px] px-1 rounded-full bg-ojas/15 text-ojas text-[10px] font-semibold tabular-nums">
                  {sourcesCount}
                </span>
              )}
            </Button>
