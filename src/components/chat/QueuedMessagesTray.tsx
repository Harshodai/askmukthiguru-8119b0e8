...
import { CornerDownLeft, Edit3, Paperclip, Trash2, Zap } from 'lucide-react';
...
export const QueuedMessagesTray: React.FC<QueuedMessagesTrayProps> = ({
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-ojas/20 bg-card/95 backdrop-blur-xl shadow-lg p-3 ring-1 ring-border/20" role="region" aria-live="polite" aria-label={t("chat.queue.regionLabel", "Queued messages")}>
        {/* Header */}
        <div className="flex items-center justify-between pb-2 mb-2 border-b border-border/60 text-xs">
          ...
          <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
            <span className="hidden sm:inline-flex items-center gap-1 font-mono">
              <CornerDownLeft className="w-3 h-3 text-ojas" /> {t("chat.queue.autoDispatch", "Auto-sends after the current response")}
            </span>
