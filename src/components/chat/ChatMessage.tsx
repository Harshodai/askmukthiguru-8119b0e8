      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm font-medium leading-5 text-foreground">
            {offer.offer_reason || t('chat.sereneMindOfferDefaultReason')}
          </p>
          <p className="mt-1 text-xs leading-4 text-muted-foreground">
            {durationLabel} · {t('chat.sereneMindOfferFooter')}
          </p>
    ...
    return (
      <div className="rounded-xl overflow-hidden shadow-md border border-border/30 bg-ojas/5 aspect-video w-full max-w-[320px] relative group">
        <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center bg-gradient-to-br from-ojas/10 to-ojas/5">
          ...
          <p className="text-sm font-medium text-ojas mb-1">{t('chat.videoUnavailable')}</p>
          <p className="text-xs text-muted-foreground mb-3">{t('chat.videoEmbedUnavailable')}</p>
          <a
            href={url}
            target="_blank"
            rel="noopener noreferrer"
    ...
    const handleSaveAsNote = useCallback(async () => {
      ...
      try {
        ...
        if (target) {
          ...
          toast({ title: t('chat.savedToNotebook'), description: t('chat.savedToNotebookDesc', { title: target.title }) });
          return;
        }
      } catch {
    ...
    const handleSaveToMemory = useCallback(async () => {
      ...
      try {
        ...
        if (err?.code === 'unauthorized') {
          toast({ title: t('chat.signInSaveMemories'), description: t('chat.memoryUnavailable'), variant: 'destructive' });
        } else {
          toast({ title: t('chat.couldNotSave'), description: err?.message ?? t('common.retry'), variant: 'destructive' });
        }
      } finally {
