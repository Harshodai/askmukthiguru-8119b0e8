...
- Repository head `d44b1c4` contains the narrow native-language peace-meaning detector fix. Backend deployment `a88a9362-a256-49c1-b654-6a9a8e314b72` reached `SUCCESS`; `/api/health` later returned `ready=true`, `status=healthy` after the expected model warm-up interval.
- Dependency-complete Railway checks passed: targeted fallback/embedding/OKF suite `27 passed, 2 skipped`; the preceding serving-focused regression set `105 passed, 3 skipped`. The serving image intentionally excludes root ingestion, ops, and security scripts; do not add corpus or maintenance files merely to make their repository tests import in production.
...
- Five exact post-release Telugu runs for `శాంతి అంటే ఏమిటి?` returned HTTP 200, grounded state, and faithfulness `0.80` with internal latency `3.65–4.49s`. This is a narrow stability proof, not a multilingual held-out benchmark.
