...
- Historical comparative release `fa41447` and its backend/worker deployments reached `SUCCESS`; it is retained as provenance, not as the current deployed head. The current backend release is recorded in the Aug 22 handoff below.
...
- The historical production baseline remains a known risk: simple English 14.67s pipeline latency, comparative English 31.39s with honest zero-source abstention, and Hindi 19.90s. Newer warm controls improved some paths, but do not claim that long-tail latency or refusal quality is solved until a held-out benchmark improves these classes.
...
### Fresh Railway handoff — Aug 23, 2026 post-audit correction
- Repository head `d5129bb` is the current local expanded-audit release before its final remote push; verify synchronization with `git rev-parse HEAD` and `git rev-parse origin/main`. Commit `4dffcb2` was deployed as `6374d867-65f8-4d7d-857d-cdc9531699c1`; direct-SSE header commit `584ff7a` was deployed as `ad84d6a6-04ce-43e5-8e8a-a0e901a7b18d`. The latter was observed as `SUCCESS` after a user-requested `railway down --yes`; the CLI reported deletion while the deployment list continued to show `DEPLOYING` and later `SUCCESS`. No later Railway deployment was started.
- The prior production-image hardening suite passed `162 passed, 2 skipped in 45.21s`. An isolated Docker build from the pushed source succeeded, and a disposable writable-memory test run passed `169 passed, 2 skipped in 38.58s`. The initial Docker failures were harness conditions: hiding `/app/memory`, mounting it read-only while tests create temporary fixtures, and patching `app.config.settings` instead of the imported `tasks.ingest_tasks.settings`.
- Grounded partials are deterministic source excerpts only: maximum two excerpts of 360 characters, absolute URLs, `verification.method=grounded_partial_evidence`, `partial=true`, `verification.passed=false`, and `faithfulness_score=0.0`. They are not verified generated teachings.
- Partial evidence may be written only to exact/hot scoped caches. Semantic/vector writes are forbidden until those tiers gain fresh evidence, source-version, and support gates. Never use a global Redis flush to address this policy.
- Direct SSE emits status/token/final/done and now sets `Cache-Control: no-cache` and `X-Accel-Buffering: no`. Queued polling also emits Redis `id:` fields, resumes from a valid `Last-Event-ID`, rejects malformed cursors, and has focused replay/authorization tests. Direct fetch-stream reconnect/resume remains unimplemented or unproven; do not generalize queued replay to direct streaming.
- Fresh bounded controls returned Serene Mind and Four Sacred Secrets as grounded partials, Beautiful State and neutral suffering as grounded `QUERY`, Hindi as grounded, and acute self-harm as blocked `DISTRESS` with no citations. These are narrow samples, not broad quality proof.
...
9. **[BLOCKER - Sep 8]** Set `FORWARDED_ALLOW_IPS` env var in Railway before next deploy. `start_railway.py` exits at startup without it (fail-closed). Set to `10.0.0.0/8` or similar RFC1918 range. Railway is currently paused — must be set before `railway up`.
10. **[BLOCKER - Sep 8]** Run full backend test suite in CI/test image. Local Docker stalls at ~41% (dependency-limited sandbox). Requires `pip install -r requirements.lock` + `pytest` in the production test image or CI pipeline.
11. **[Sep 8]** NDCG integration test requires production Qdrant access. Cannot run locally without loading production env via `railway run`. Blocked on Railway unpause.
12. **[Sep 8]** Google login E2E and Forgot-password E2E require real Supabase email delivery. Cannot run in sandbox — needs Supabase project with `mailer_autoconfirm: false` and working email provider.
13. **[Sep 8]** Guru-voice benchmark requires live LLM run with `langhanam_voice_enabled=true`. Blocked on production deployment + benchmark infrastructure.
...
### Cost-efficiency hardening — 2026-08-22
- Commit `34c98a5` adds privacy-bounded OpenRouter accounting metrics. Provider-reported `usage.cost`, known fallback estimates, unknown-cost events, prompt-cache reads, and prompt-cache writes are separate signals. Unknown cost must never be treated as free.
- Live production override is `OPENROUTER_GENERATION_MODEL=google/gemini-2.5-flash`, not the configured default. The fallback table covers Gemini 2.5 Flash at $0.30/M input and $2.50/M output and Gemini 3.6 Flash at $0.75/M input and $3.75/M output; provider-reported usage always wins.
- Backend and worker were deployed from a clean archive as deployment IDs `888e9f55-a087-4680-ab45-a5fa215fa7d1` and `c53d951d-6c9c-4525-99c7-dae894c60ed6`. Health returned healthy with Neo4j/Qdrant/Redis healthy, embedding dimension 1024, graph healthy, and queue size 0.
- Latest Railway usage snapshot: $29.3763 current period, $27.6770 memory (94.2%), $1.3536 CPU (4.6%), $0.2685 volume (0.9%), $0.0772 egress (0.3%), and $54.7602 estimated bill. The workspace hard limit is $30; do not run broad production benchmarks while this limit is nearly exhausted.
- Latest 30-minute service telemetry: backend average 6,767.6 MB and max 13,634.7 MB; Neo4j average 2,399.7 MB; worker average 177.4 MB. These are capacity signals, not proof that any particular model/cache owns the memory.
- Post-deploy logs show BGE-M3 is fetched at startup and the reranker is not in the image cache; OKF compiled index and doctrine lexicon are absent in the deployed archive. Do not hide these warnings or create empty artifacts. Treat them as separate image/reingestion follow-up work; never modify `scripts/ingestion/corpus/`.
- Cost-saving changes remain evidence-gated: do not switch ONNX INT8, tune RRF/DBSF, mutate Neo4j schema, increase broad graph concurrency, or reduce caches solely from a theoretical dollar estimate. Required proof is held-out quality/safety/privacy plus p95/p99 latency, RSS, provider-call, and cost-per-successful-grounded-answer evidence.
- Production smoke after deployment was healthy: Hindi and Telugu were grounded with faithfulness 0.8, distress was a safety redirect, but concurrent wall-clock was still long (roughly 18–23 seconds) despite lower internal route timings. Treat this as an open capacity/queueing issue, not a solved latency target.
- Local sandbox remains dependency-limited: `py_compile`, regex safety, diff, and corpus guards passed; `pytest` is unavailable and no packages may be installed merely to force a local pass.
...
- The dominant broad-scale cost blocker remains memory: latest measured Railway usage was `$29.3763` against a `$30` hard limit, with `94.2%` memory share. No ONNX INT8, RRF/DBSF, Neo4j mutation, broad graph concurrency, or global Redis eviction may be activated without the documented held-out evidence gates.
- `Dockerfile.railway` must not pre-cache or imply active ONNX while production uses FlagEmbedding-compatible Qdrant vectors. Runtime model downloads, missing OKF compiled index, and missing doctrine lexicon are packaging/cold-start findings; do not create empty placeholders or modify `scripts/ingestion/corpus/`.
- Local `pytest`, Vitest, and TypeScript dependencies are unavailable and must not be installed ad hoc. Valid local checks are Python compilation, JSON validation, `git diff --check`, corpus immutability, and bounded live health/SSE/browser probes.


## Final handoff — Aug 23, 2026

- Explicitly authorized Second Brain E2E completed in the connected Lovable browser: the labelled disposable reflection `Disposable production verification — delete me` was created, confirmed after reload, deleted with `Forget this`, and confirmed absent after a settled reload. The route also unlocked successfully after the final BRAIN_KEK process refresh.
- Explicitly authorized BRAIN_KEK rotation completed without printing or committing key material. Dry-run validated 2 Mode-A rows; CAS apply rewrapped exactly 2 rows. The replacement was promoted to backend and worker `BRAIN_KEK`, `BRAIN_KEK_NEXT` was deleted from both scopes, and both processes were explicitly redeployed again afterward. Final backend deployment `108888e3-757d-4a37-9c77-0a890c5a51c4` and worker deployment `f9d05731-6fb0-4b8d-92bd-21fb14b01aa9` reached `SUCCESS`; `/api/health` returned ready/healthy with embedding dimension 1024. A name-only audit confirmed BRAIN_KEK present and BRAIN_KEK_NEXT absent in both scopes.
- Post-rotation tests: image-compatible Second Brain/crypto/rotation coverage `17 passed in 16.63s`; full serving-focused suite `83 passed, 2 skipped in 29.43s`. Repository-only schema tests remain separate because the flattened serving image intentionally excludes `/supabase/migrations`.
- Current repository head is `c0bbd81`; worktree is clean and the immutable `scripts/ingestion/corpus/` directory remains untouched. Strict verdict remains NOT READY because reviewed OKF/doctrine runtime artifacts are absent, cost/memory and long-tail latency gates are open, Lovable SSE-final publication is unproven, broader auth/RLS/upload/recovery checks remain incomplete, and custom-domain/browser breakpoint coverage is not closed.

### Docker latency hardening — 2026-08-23
- A bounded Docker benchmark through the host gateway reached Qdrant, Redis, Neo4j, and FastAPI. Across 35 dry-run/no-flush requests, the local benchmark reported `p50=279.5ms`, `p95=1,343ms`, `p99=3,167ms`, and `max=4,403ms`. The readiness score was `45%` because the local selected Qdrant collection has zero points and provider/runtime quality is not production-equivalent; do not treat this as a production benchmark.
- The queued SSE consumer emitted `final` and `done` quickly but did not return on `done`; it waited for another `XREAD(..., block=2000)` cycle because the worker emits `final`/`done` without a `__COMPLETE__` sentinel. The consumer now returns immediately after the authoritative `done` event.
- The frontend non-stream queue fallback no longer sleeps a fixed two seconds before its first poll. It polls immediately and then backs off from `250ms` to a maximum of `1s`.
- Dependency-complete Docker validation passed the backend safety/grounding/chat groups plus the new queued-SSE regression: `88 passed in 7.87s`. Docker frontend transport validation passed `9 tests`, and TypeScript typecheck passed. After backend restart, queued Docker probes completed in `364ms` and `1,704ms`, with first events at `95ms` and `1,118ms`; this is local transport evidence only.
- These changes are latency mitigations, not production SLO closure. Production p95/p99, provider/session/admission overhead, multilingual and comparative tails, and cost impact remain open. No Railway deployment was started, no corpus files were changed, and the stopped worker was restored only temporarily for the local queue probe.


### Docker retrieval and local feature audit — 2026-08-23
- The full local retrieval-quality gate ran in a root-mounted dependency-complete Docker test image: `5 passed, 4 skipped`. The four NDCG integration strategies did not execute because the reachable local `spiritual_wisdom` Qdrant collection has `0` points. A metadata-only flagship probe returned `QUERY`, `no_context_short_circuit`, zero citations, and a 38-character final for both Serene Mind and Four Sacred Secrets. This is a local data-plane failure, not proof of a production quality regression or closure.
- The local Docker feature sweep passed `179 tests in 8.99s` across assistants, personal memory, graph, Second Brain, tenant, authorization, and related contracts. Read-only counts were `second_brain_vault=0`, `global_memory=0`, zero scanned Redis keys for inspected memory/Second Brain/session/job patterns, Neo4j `80` nodes/`88` relationships, public ontology `50` nodes/`57` edges, and `0` visible assistants. Authenticated memory/Second Brain E2E and Postgres persistence were not claimed.
- Component timing in Docker: cold embedding initialization approximately `8,628ms`; warm full encoding `82ms`, `0ms`, `0ms`; hybrid Qdrant search on the empty collection `11.1ms`, `1.8ms`, `1.7ms`; fresh OpenRouter generation `1,548ms` ending HTTP `401`; no local Ollama endpoint reachable. Do not generalize empty-collection retrieval timings to production.
- No corpus files, user data, or Railway deployment were changed during this audit. Temporary probes were removed from the desktop worktree. The local feature and retrieval findings are evidence boundaries, not production readiness closure.


### Expanded Docker interaction audit — 2026-08-23
