...
- **L-LAT-5 — Production proof.** The bounded comparison on the live Railway endpoint returned 367 characters in approximately 7ms with `route_decision=bounded_comparison_short_circuit`, no citations, `grounding_state=abstained`, and `verification.method=limited_comparison_fallback`. After the translation deadline change, an English FAQ under Hindi preference completed in approximately 4.3s and a native Hindi FAQ completed in approximately 4.0s, compared with the earlier ~17–22s tail. Health reported `ready=true`, `status=healthy`, Qdrant/Redis/Neo4j/LLM/embedding/fast-graph/LightRAG checks healthy, embedding dimension 1024, exact-cache keys 0, queue size 0, and chat backpressure clear.
