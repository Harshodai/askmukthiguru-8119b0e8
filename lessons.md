...
- **Solution**: Refactored the `handleGoogleSignIn` function in [AuthPage.tsx](file:///Users/harshodaikolluru/Public/askmukthiguru-8119b0e8/src/pages/AuthPage.tsx) to check the `VITE_USE_NATIVE_OAUTH === 'true'` environment flag. If true, it invokes `supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } })`. The Lovable-managed `lovable.auth.signInWithOAuth('google')` code block was kept but commented out for future reference.
- **Lesson learned**: When custom branding and native Supabase OAuth control are required, the frontend authentication flow must be conditionally routed directly to the native Supabase provider, bypass managed third-party proxies, and specify the correct authorized redirect URIs.
