...
- **Indic Phonetic Alignment**: Refined the `IndicPhoneticMatcher` metaphone encoding replacement to correctly map spelling variations like `"khsh"` as a Sanskrit `"ksh"` replacement to `"x"`. This ensures that phonetic searches collapse variations like `"dikhsha"`, `"diksha"`, and `"deeksha"` to the identical phonetic key `"DIXA"`, enabling bulletproof Indic misspelling tolerance.
