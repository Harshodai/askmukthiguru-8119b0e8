...
### RULE 45 — Four divergent "is this a real user" checks across the codebase, each guarding a different Postgres/vault call, three of them wrong

**Problem**: Live logs showed, on every single anonymous chat request: `invalid input syntax for type uuid: "anon:<uuid>"` (code `22P02`) from three separate Supabase call sites, plus `Second Brain recall timed out for user anon:<uuid> (exceeded 200ms budget)` on every request with a Second Brain service configured. None of these crashed the request (all caught non-fatally), but every anonymous turn — the default mode per `CLAUDE.md`'s incognito-by-default design — was paying for 3-4 guaranteed-to-fail external calls before falling back to the in-memory path anyway.

**Root cause**: anonymous sessions resolve to `anon:<session_id>` (`resolve_anon_identity`, root `CLAUDE.md` caching invariants), not the literal string `"anonymous"`. Four places in the codebase check "is this user real enough to write to Postgres," and only one was doing it correctly:
- `services/user_profile_service.py::_is_persistable_user_id()` — **correct**: actually parses `uuid.UUID(str(user_id))`. Used by `save_conversation_memory`, but NOT by the sibling `get_or_create_profile`/`update_profile` in the same file, which had **no guard at all**.
- `services/memory_service.py::MemoryService._is_anonymous()` — **wrong**: `not user_id or user_id == "anonymous"`. Guards 11 call sites in this one file (`get_core`, `search_semantic`, and 9 more), every one of them broken for anonymous traffic.
- `app/orchestrator_utils.py`'s Second Brain recall block — **no guard at all**: `if getattr(container, "second_brain", None) is not None:` unconditionally attempted `vault.unlock(user_id)` for every request, burning up to its own 200ms timeout budget on a call that can never succeed for a non-UUID id.
- `app/pipeline/stages/memory_stage.py`'s Second Brain write block — **same shallow bug as memory_service.py**: `user_id and user_id != "anonymous"`, missing every `anon:` session, firing a background `asyncio.create_task` that was guaranteed to log `Second Brain unlock failed (non-fatal)` every time.

**Fix**: rather than write a fifth divergent check, imported the one correct implementation (`_is_persistable_user_id`) into the three broken call sites and fixed `memory_service.py`'s `_is_anonymous` in place (one shared method, 11 call sites corrected simultaneously). Four test files had fixtures using non-UUID placeholder ids (`"user123"`, `"user-1"`, `"u1"`) that the old buggy checks let slide through unnoticed — updated to real UUIDs so the tests exercise the actual guarded path again, rather than silently no-op.

**Rule**: when a "should this touch the database" guard exists in more than one place, grep for every variant before writing a new one (`grep -rn "== \"anonymous\"\|_is_anon\|is_persistable"`) — a second, subtly different implementation of the same check is a sign the first one was never centralized, and each copy silently drifts. A guard that's *slightly* wrong (excludes the literal string but not the actual runtime value) is worse than no guard, because it looks correct in review and only fails against real traffic shapes a unit test's placeholder fixture never produces.

---

### RULE 46 — A renamed function's dead call site ran silently on every single answer, forever, because the `except Exception` around it was too wide

**Problem**: `rag/nodes/generation.py`'s "A3: Humanizer" step — a deterministic, config-gated (`strip_canned_footer`, default `True`) pass meant to strip AI-writing tells (sycophantic openers, hedging, filler) from every generated answer — logged `Humanizer skipped (non-fatal): cannot import name 'apply_humanizer_to_response' from 'services.humanizer'` on every request, silently, presumably since whatever refactor renamed the real function to `scrub()`/`scrub_with_report()` in `services/humanizer.py`. The step has likely never run once in production.

**Root cause**: the call site did `from services.humanizer import apply_humanizer_to_response` **inside** a `try: ... except Exception as _humanizer_err: logger.warning(...)` block — a defensible pattern for "this enhancement is optional, never let it break the answer," but the same wide `except Exception` also swallows `ImportError`/`AttributeError` from a stale reference, which is a real bug, not a runtime hiccup. The warning-level, "(non-fatal)" framing of the log line made it look like expected degradation rather than a dead feature.

**Fix**: called the real function, `scrub(answer)` — it takes no `intent` argument (the module's own docstring is explicit that it's deterministic regex, no LLM, no intent branching, by design), so the fix is a straight one-line replacement, not a signature migration.

**Rule**: a `try/except Exception` wrapping an inline `import` is indistinguishable, from the logs alone, between "this optional dependency isn't installed" (fine) and "this symbol doesn't exist anymore" (a real regression) — the latter is a `NameError`/`ImportError` on literally every call, forever, not a transient failure. When a "skipped (non-fatal)" warning appears on 100% of requests rather than intermittently, that ratio itself is the signal it's not really optional-and-occasionally-unavailable — it's permanently broken.

---

### RULE 47 — Jaeger was never deployed on Railway; every request paid multi-second OTLP export retries into the void

**Problem**: every request logged 2-4 `Transient error StatusCode.UNAVAILABLE encountered while exporting traces to jaeger:4317, retrying in Ns` lines with increasing backoff (0.99s, 1.08s, 1.10s, 2.20s...) followed by a final `Failed to export traces... Domain name not found`. Purely log noise on the surface, but each failed batch's retry sequence runs real wall-clock time on a background thread in a single-worker (`WEB_CONCURRENCY=1`) process — and Railway bills CPU/RAM by actual per-second consumption (confirmed via research: Railway's model is metered resource-based billing, not reserved capacity — see Cost Optimization section below), so this was small but *real*, *permanent*, *self-inflicted* spend on every single request since whenever Jaeger was last actually running.

**Root cause**: `app/observability.py`'s `init_observability()` defaults `endpoint = os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://jaeger:4317")` and is gated by `OTEL_ENABLED` (default `true`) — but no `jaeger` service exists in this Railway project (confirmed via `railway service list`: `askmukthiguru-8119b0e8`, `celery-worker`, `gb-neo4j-railway-template`, `qdrant`, `Redis` — no jaeger). The exporter was faithfully doing its job against a collector that was never provisioned in this environment (it's referenced in the root `CLAUDE.md`'s local `docker compose up -d qdrant redis neo4j jaeger` — a dev-only service that never made it to prod).

**Fix**: `railway variable set OTEL_ENABLED=false --service askmukthiguru-8119b0e8` — no code change needed, since the module was already correctly designed to degrade cleanly when disabled. This is the honest fix: disable an exporter with nowhere to export to, rather than let it retry forever. Re-enable if/when a real Jaeger service is provisioned in this environment.

**Rule**: a `docker-compose`-only sidecar referenced by a `getenv(..., "http://sidecar:PORT")` default is a trap the moment the code ships to an environment that doesn't run that sidecar — the failure is *silent* (caught, logged, retried) rather than a startup crash, so it can run in production indefinitely without anyone noticing it's pure overhead. Before believing a background export/telemetry feature is "just a bit noisy in the logs," check whether its target actually exists in the deployment (`railway service list`) — if it doesn't, that's not noise, it's wasted compute on every request, forever.

---

### RULE 48 — `railway status`'s `latestDeployment` field returned a week-old cached record; `railway deployment list --json` was the reliable source of truth

**Problem**: mid-incident, `railway status` reported the backend's `latestDeployment` as `status: FAILED`, `id: 48861e1e...`, causing real alarm (was this a new, current failure?). Cross-checking with `railway deployment list --service <name> --json` showed the actual most recent deployment was a completely different ID, created *today*, with `48861e1e`'s own `createdAt` timestamp reading **2026-07-17** — over a week stale. `railway status`'s plain-text mode had shown similar staleness earlier in the session too (repeatedly reporting a deployment ID that hadn't changed across several genuinely-different redeploys, and once returning byte-identical log content across two calls minutes apart).

**Root cause**: not fully diagnosed (this is a CLI/API-side caching behavior, not something in this repo's control), but empirically: `railway status`'s GraphQL query for a service's `latestDeployment` field is not reliably fresh. `railway deployment list --json`, which lists the full deployment history with real timestamps, was consistently accurate throughout the same incident window.

**Rule**: when `railway status`'s single `latestDeployment` field disagrees with reality (a status that doesn't match what logs/health checks show), don't trust it in isolation — cross-check against `railway deployment list --service <name> --limit 5 --json` and compare `createdAt` timestamps before concluding anything is actually broken. Prefer `--json` output generally: plain-text `railway logs`/`railway status` showed more staleness artifacts this session than their `--json` equivalents, though even JSON log queries with `--since`/`--filter` time-bounding sometimes returned empty against a since-stopped or since-recreated service — when a JSON log query returns nothing, that alone doesn't mean "no logs exist," it can mean "this specific query shape isn't hitting live data right now." Fall back to a plain, unbounded `railway logs --lines N` (still-linked service context, no `--service` override) as the last-resort check that consistently surfaced real content this session.

---

### RULE 49 — Railway's Hard Limit (Shutdown) took every service in the project offline simultaneously; two of five never came back on their own

**Problem**: mid-session, the public backend URL started returning `{"status":"error","code":404,"message":"Application not found"}` — a Railway **edge/proxy**-level response (not the app's own JSON health schema), meaning no deployment existed to route to at all. The user confirmed their Railway usage/spend limit was exhausted.

**What actually happens** (confirmed via research, [Railway Usage Limits docs](https://docs.railway.com/reference/usage-limits)): Railway offers two limit types — a **Soft Limit (Alert)**, which only emails you and leaves resources untouched, and a **Hard Limit (Shutdown)**, which takes **all workloads in the account offline** the moment the limit is hit, specifically to stop further resource consumption. Recovery is supposed to be automatic: *"Railway will automatically redeploy your stopped services once the limit is raised or removed."*

**What was observed, once the limit was presumably raised/lifted**: `askmukthiguru-8119b0e8` (backend) and `celery-worker` both came back cleanly and automatically the moment a new deploy was triggered (`railway up` completed BUILDING → DEPLOYING → SUCCESS in under 2 minutes, no manual intervention needed beyond that one push). But **`gb-neo4j-railway-template` and `qdrant` did not self-recover** — Neo4j sat in `status: FAILED, deploymentStopped: true, replicas.running: 0` until manually redeployed (`railway redeploy --service gb-neo4j-railway-template --yes`, which worked and started a fresh `DEPLOYING` cycle), and **Qdrant had no deployment record at all** (`deploymentId: null, latestDeployment: null` — `railway redeploy` returned `"No deployment found for service"`, a state the CLI can't recover from). Both services' data volumes remained intact and `READY` (Neo4j 1.4GB/50GB, Qdrant 2.3GB/50GB) — only the running container/deployment was lost, not the underlying storage.

**Rule**: "automatic redeploy on limit raise" is not guaranteed uniform across every service type in a project — template/managed-image services (a community Neo4j template, a `qdrant/qdrant:v1.17.1` image with no connected git repo) appear less likely to self-heal than git-connected app services that redeploy from a repo. After a Hard Limit shutdown is lifted, **check every service individually** (`railway service list --json`, look at `status`/`deploymentStopped`/`replicas.running` per service) rather than assuming the one service you actively redeployed proves the whole project recovered. A service showing `"No deployment found for service"` on `redeploy` needs a fresh deploy triggered a different way (Railway dashboard "Deploy" button, or recreating the service pointed at the same existing volume) — the CLI's `redeploy` command specifically requires an existing deployment record to redeploy *from*, and has no path to originate a brand-new one for an image-based service.

---

## Cost & Pipeline Optimization (ruthless pass, 2026-07-26)

Grounded in real Railway pricing research and this session's own findings — not generic advice. [Sources: Railway pricing overview](https://makerkit.dev/pricing-calculator/railway), [Railway pricing explained](https://livemy.app/blog/railway-pricing), [Railway usage limits](https://docs.railway.com/reference/usage-limits).

**How Railway actually bills** (this changes what's worth optimizing): resource-based, metered-per-second, not reserved capacity — CPU ≈ $20/vCPU/month (~$0.000463/vCPU-minute), RAM ≈ $10/GB/month (~$0.000231/GB-minute), egress ≈ $0.10/GB. Critically: **you pay for what containers actually consume, not the declared limit** — raising `railway.json`'s `deploy.limits.memory` from 6Gi to 16Gi (RULE 42-adjacent, done this session to give real OOM room below the platform's 24GB ceiling) does not itself cost more; it's a safety ceiling, not a reservation. But **idle containers still cost money** — a service that's provisioned and running 24/7 bills continuously regardless of whether it's doing useful work at that moment.

**Confirmed-fixed cost leaks (this session)**:
1. Jaeger OTLP export retries on every request into a nonexistent collector (RULE 47) — multi-second background-thread activity per request, forever, for zero benefit. Fixed (`OTEL_ENABLED=false`).
2. 3-4 guaranteed-to-fail Supabase/Second-Brain calls on every anonymous request (RULE 45) — real external round-trips (Supabase has its own request-based billing too) plus up to 200ms of deliberate timeout-waiting, on what is likely the majority of traffic given incognito-by-default design. Fixed.
3. `UserProfileService`'s unbounded in-memory caches (RULE 41, prior session) and `CacheUpdateStage`'s blocking event-loop call (RULE 42) both caused real production crash-and-restart cycles — every crash-restart is itself extra compute (full re-init: reload BGE-M3, reranker, LettuceDetect from disk, rebuild LangGraph, reconnect all six external services) on top of whatever caused it. Fixed and verified stable — 24h memory metrics post-fix: avg 5.27GB, max 6.68GB, 27% of the 24GB platform ceiling, flat (no leak).

**Open, real levers, in priority order** (not yet acted on — flagging for a deliberate decision, not blind action):

1. **`railway.json` declares `cpu: 4000` (4 vCPU)** while the app is architecturally single-threaded at every level that matters: `WEB_CONCURRENCY=1`, `OMP_NUM_THREADS=1`, `MKL_NUM_THREADS=1` (all explicitly set, presumably for exactly this reason — predictable memory behavior over throughput). A single asyncio process with numeric-thread envs pinned to 1 cannot sustain genuine 4-core utilization except for brief native-library bursts (numpy/torch's own internal parallelism during embedding/rerank calls, which ignore the `OMP`/`MKL` pins only partially). Since Railway bills metered actual usage, an over-declared CPU *limit* likely isn't directly costing extra by itself (same logic as the memory limit) — but it's worth requesting Railway's own per-service cost breakdown (dashboard → service → usage) to confirm actual CPU-minutes billed vs. what 1-2 vCPU would need, before deciding whether to lower the declared limit.
2. **`celery-worker` runs 24/7** whether or not there's active ingestion work. `QUEUE_ENABLED=false` disables `job_queue`, but a separate `llm_queue_enabled` (default `True`, not overridden in production vars per the earlier remediation plan's own finding) likely keeps it consuming LLM-queue tasks continuously. This was flagged as needing verification *before* touching it as far back as this session's own earlier remediation plan ("Before touching celery-worker: pull Railway deploy logs/metrics over a real window to confirm actual task throughput. Only downsize/remove if genuinely idle — do not assume") — that verification was never completed. It remains the single highest-value next step: if genuinely idle most of the time, this is a real, avoidable 24/7 cost with a known Railway-recommended fix (scale-to-zero / sleep for non-critical services).
3. **Benchmark runs are a direct, non-trivial cost event, not just a diagnostic.** Each full 373-question run against the live backend makes ~373-1000+ real OpenRouter LLM calls (standard/deep-tier queries invoke multiple sub-steps: intent classification, generation, verification/reflection) *and* consumes 15-20 minutes of real Railway compute-seconds. This session ran the full suite multiple times, including twice against the same backend within an hour. Recommendation: default to `--category X --limit N` scoped runs for iteration; reserve the full 373-question sweep for deliberate pre-release validation, not routine "let's check if it's fixed" runs — a 3-question canary in the affected categories answers "did the fix work" for a fraction of the cost.
4. **OpenRouter (the actual LLM provider cost) is very likely the dominant variable cost line, not Railway compute at all** — every "Standard"/"Deep" tier chat query fans out into several separate model calls (intent, generation, verification), and this wasn't directly measured this session. Worth a dedicated pass: pull OpenRouter's own usage dashboard, correlate spend against query tier (Fast/Standard/Deep) and against benchmark-vs-real-traffic volume, before assuming Railway is the primary cost surface at all.
5. **Consolidation**: Neo4j, Qdrant, Redis, and the backend are four always-on services in one project; per the pricing research's own advice ("consolidate databases... a separate Postgres per project each holds memory 24/7"), each additional always-on service is a separate continuous cost line. Given Neo4j's actual data footprint is ~1.4GB against a provisioned 50GB volume and modest 1G heap/512M pagecache (already right-sized earlier this session), this is likely already reasonably lean — flagging only as a category to re-check if Railway's own cost breakdown shows otherwise, not a recommendation to act on blindly.

**Not recommended**: don't lower `PYTHON_MEMORY_LIMIT_MB` back down as a cost-saving move — that ceiling exists to prevent a hard container OOM-kill (RULE 41/42/49 context), and Railway bills actual usage, not the ceiling; lowering it only reintroduces the crash-loop risk for zero cost benefit.
...
### Grounded guru voice must be authored at generation time, never retrofitted after retrieval
Post-generation LLM rewriting and regex substitutions can silently change the speaker, dilute source attribution, and make a response appear more certain than its evidence supports. Keep the voice instruction in the generation prompt; preserve a founder-style first-person formulation only where the cited material actually supports that voice; otherwise use clear attribution such as “Preethaji and Krishnaji point to…”. Keep compatibility adapters as no-ops and cover both the prompt contract and no-rewrite invariant with regression tests.
### A response-quality signal should expose provenance, not simulate certainty
A compact response indicator may show source count and verifier confidence only when those values originate in the response metadata. It must provide an accessible control for opening sources and must never invent a confidence or imply that a low-evidence response is authoritative. The profile preview should explain how tone, depth, and attribution preferences affect guidance before the user saves them.
### Release validation needs an explicit dependency-install step and a production-preview accessibility gate
Run `npm ci` after changing `package-lock.json`; test and audit the dependency tree that will actually be installed, rather than a stale `node_modules`. Run the production build before Playwright, and point E2E configuration at the preview server (port 4173 in this project) to avoid development hot-reload behaviour masking accessibility defects. The release gate used here comprised the full backend suite, focused frontend regressions, build/prerender, axe WCAG 2 A/AA checks, an F821 scan, `npm audit --omit=dev`, and `git diff --check`.
### A clean release review must include untracked documents and staged whitespace checks
...
## Latency-complete pass — 2026-08-22

- **L-LAT-1 — Short-circuit known bounded comparisons after safety checks.** A narrowly recognized English meditation-versus-contemplation comparison now returns the clearly labelled limited-support answer after input and distress guardrails, before doctrine cache, retrieval, graph, grading, or generation. This removes a measured ~25–29s path for a request that was already destined for the same bounded fallback. The shortcut is intentionally English-only so it cannot bypass the established Indic translation contract, and it is placed after distress detection so a safety signal always wins.
- **L-LAT-2 — Do not translate an English message merely because the UI preference is Indic.** `detect_and_prepare_language_info()` now checks the message script/language before setting `should_translate`. This removes an unnecessary provider round trip for users who select Hindi/Telugu/etc. but type in English; native or code-switched non-English messages retain the translation path.
- **L-LAT-3 — Provider translation is a bounded dependency.** Query, history, and final-answer translation calls now use `translation_timeout_s` (default 5s) and fail open with the original/native text or the English answer when the provider exceeds the deadline. BGE-M3 is multilingual and the generation model is multilingual, so a slow translation service must not hold the full chat request hostage. Timeouts are logged at warning level and are not silently swallowed.
- **L-LAT-4 — Evidence before retrieval changes.** ONNX INT8 query/index embedding migration remains disabled because the existing Qdrant vectors were produced with fp32. RRF/DBSF weights and prefetch multipliers remain unchanged until a held-out matrix proves NDCG/recall, faithfulness, citation correctness, abstention, p95 latency, and error-rate non-regression. Neo4j schema mutation and broad graph fan-out remain maintenance/evaluation-gated; graph retrieval must continue to fail open.
- **L-LAT-5 — Production proof and deployment boundary.** The active Railway endpoint returned the bounded comparison in approximately 7ms with 367 characters, `route_decision=bounded_comparison_short_circuit`, no citations, `grounding_state=abstained`, and `verification.method=limited_comparison_fallback`. The same active runtime returned an English FAQ under Hindi preference in approximately 4.3s and a native Hindi FAQ in approximately 4.0s, while the benchmark matrix also captured a 10.8s pipeline/13.8s wall-clock Hindi run, proving that multilingual latency remains variable. The attempted `940eb55` rollout was not active: Railway marked it FAILED during `Initialization › Snapshot code` after 15 minutes, with Build/Deploy never started; the dashboard continued to show `8655709` as ACTIVE. Health reported `ready=true`, `status=healthy`, Qdrant/Redis/Neo4j/LLM/embedding/fast-graph/LightRAG checks healthy, embedding dimension 1024, exact-cache keys 0, queue size 0, and chat backpressure clear. The translation-timeout code is now production-proven through the successful `806799d` rollout; its remaining limitation is tail variability rather than deployment uncertainty.
- **L-LAT-6 — Test-environment honesty.** Python compilation, regex/ReDoS safety, `git diff --check`, and corpus immutability gates passed. The focused pytest suite could not run because `pytest` is not installed in the sandbox; no package was installed because the task explicitly prohibited installation. This is a release limitation, not a passing test claim.

### Gated optimization disposition

Do not treat a lower wall-clock number as sufficient evidence for a retrieval or graph change. The required held-out gate is documented in `docs/LATENCY_EVIDENCE_GATES.md`. Until the corpus-compatible ONNX index migration, RRF comparison, Neo4j schema plan, and graph-parallel canary each produce the required quality and tail-latency evidence, production stays on fp32 query/index embeddings, current fusion settings, current Neo4j schema, and bounded graph behavior.


## Multilingual quality follow-up — 2026-08-22

The final smoke matrix exposed a remaining quality defect: the Telugu query `శాంతి అంటే ఏమిటి?` returned a 52-character Telugu refusal even though one source citation and a language-aware faithfulness score of 0.80 were reported. The generator now recognizes the observed Telugu refusal variants and performs one evidence-aware retry; the fix is committed in `899b17c`/`4833a95` but is not yet production-proven because Railway failed the rollout during code-snapshot initialization. The active runtime remains `8655709`, so the short-term benchmark result must be treated as a live regression signal rather than a closed defect.

Railway's dashboard diagnosis is more specific than the CLI error: `940eb55` failed after 15 minutes at `Initialization › Snapshot code`, with Build, Deploy, Network, and Post-deploy never starting. The same control-plane failure affected subsequent 899b17c/4833a95 attempts, while `/api/health` and `/api/healthz` remained healthy on the prior active runtime. Do not use `railway down`, global cache eviction, or a rollback action without a known-good replacement; the active deployment is still serving users.


## Post-rollout evidence — 2026-08-22

- **L-LAT-7 — Quality retry verified in production.** After `806799d` reached SUCCESS on both Railway application services, the Telugu query `శాంతి అంటే ఏమిటి?` returned a 289-character grounded answer with one citation, `faithfulness_score=0.8`, and `language_aware_fast_tier` verification instead of the previous 52-character refusal. The one-shot native-language evidence retry is therefore a measured quality improvement, not just a unit-level change.
