...
- Quota storage is a port (`services/anon_quota_port.py`) with Redis (`services/anon_quota_redis.py`) and in-memory (`services/anon_quota_memory.py`) adapters; `AnonQuotaService` chooses Redis when available and falls back to memory. The in-memory adapter is process-local: state does not survive restart and is not shared across replicas — sandbox/single-node dev only. The Redis adapter is the only production-safe path.
