...
def run_evaluation():
    ...
    if "OPENAI_API_KEY" not in os.environ:
        logger.warning(
            "OPENAI_API_KEY not set. Ragas uses OpenAI by default for evaluation metrics."
        )
        logger.warning(
            "Please set OPENAI_API_KEY or configure Ragas to rely on the local Ollama LLM."
        )
