...
def _grounded_response() -> dict:
    return {
        ...
        "guidance_plan": {
            "attribution": {"label": "Guidance inspired by retrieved teachings", "source_backed": True},
...
def test_priority_language_fixture_has_complete_approved_coverage():
    dataset = load_dataset(__import__("pathlib").Path(__file__).parents[1] / "evaluation/datasets/priority_languages_v1.json")
    assert validate_dataset(dataset) == []
    assert {item["language"] for item in dataset["items"]} == {"en", "hinglish", "hi", "te", "ta", "kn"}


def test_grounded_case_requires_evidence_and_a_practical_next_step():
    item = {"id": "case", "language": "en", "requires_source_evidence": True, "expects_guidance": True}
...
def test_crisis_case_rejects_a_guidance_plan():
    ...
    response = {"response": "Please contact local emergency support and someone you trust right now.", "guidance_plan": {"attribution": {"label": "bad"}}}
