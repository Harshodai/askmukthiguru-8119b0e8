...
def test_slo_latency_observed():
    ...
    assert any("observe_slo_latency(" in ln for ln in lines), (
        "no callsite routes through observe_slo_latency"
    )
    ...
    assert not any(".observe(" in ln for ln in raw_lines), (
        "raw SLO histogram observed outside metrics.py"
    )
...
def test_coordinator_path_counts_violation_when_over_threshold():
    ...
    assert after == before + 1, (
        "coordinator-path over-threshold request must increment violation counter"
    )
