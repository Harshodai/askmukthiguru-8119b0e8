...
def test_all_11_pilot_fixture_quality_evaluations():
    ...
    manual_segs = [
        CanonicalSegment(segment_id="seg_0", start=0.0, end=5.0, text="Welcome to Ekam.", source_tier="manual_api"),
        CanonicalSegment(segment_id="seg_1", start=5.0, end=10.0, text="We explore the Beautiful State.", source_tier="manual_api"),
    ]
    rep = engine.evaluate_quality("vid_manual", manual_segs, duration_seconds=10.0, review_completed=True)
    ...
    auto_segs = [
        CanonicalSegment(segment_id="seg_0", start=0.0, end=5.0, text="Finding inner peace and stillness in the present moment.", source_tier="auto_api")
    ]
    rep_agree = engine.evaluate_quality("vid_auto", auto_segs, duration_seconds=5.0, asr_comparison_text="Finding inner peace and stillness in the present moment.")
    assert rep_agree.quality_state == "trusted"
    assert rep_agree.caption_asr_agreement >= 0.70

    # 3. High disagreement between caption & ASR
    rep_disagree = engine.evaluate_quality("vid_disagree", auto_segs, duration_seconds=5.0, asr_comparison_text="Completely different unrelated words with zero overlap.")
    assert rep_disagree.quality_state == "needs_review"
    assert "caption_asr_disagreement" in " ".join(rep_disagree.flags)

    # 4. Music / Sound-only
    music_segs = [
        CanonicalSegment(segment_id="seg_0", start=0.0, end=10.0, text="[Music playing]", source_tier="pilot_mock", is_non_speech=True)
    ]
    ...
    # 5. Silence
    silence_segs = [
        CanonicalSegment(segment_id="seg_0", start=0.0, end=10.0, text="", source_tier="pilot_mock", is_non_speech=True)
    ]
    ...
    malformed_segs = [
        CanonicalSegment(segment_id="seg_0", start=10.0, end=15.0, text="First line.", source_tier="pilot_mock"),
        CanonicalSegment(segment_id="seg_1", start=5.0, end=8.0, text="Backwards jump.", source_tier="pilot_mock"),
    ]
...
def test_artifact_manifest_structure_and_hashing(tmp_path):
    ...
    segs = [
        CanonicalSegment(segment_id="seg_0", start=0.0, end=4.0, text="Welcome to Ekam.", source_tier="manual_api")
    ]
