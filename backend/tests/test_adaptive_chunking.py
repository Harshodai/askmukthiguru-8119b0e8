...
def test_intrachunk_cohesion():
    ...
    low_cohesion_chunk = "This is a spiritual teaching. I bought a red apple. The server runs on port 8000."
...
def test_chunk_document_routing():
    ...
    doc_text = "This is a long spiritual document about self-realization and freedom from suffering."
    
    # Mock split methods to return predictable lists
    service._split_recursively = MagicMock(return_value=["Chunk 1", "Chunk 2", "Chunk 3"])
    service._split_semantically = MagicMock(return_value=["Huge chunk that takes up almost the whole text", "tiny"])
