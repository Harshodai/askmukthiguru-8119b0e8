from unittest.mock import AsyncMock, patch

import pytest

from rag.nodes.intent import handle_distress
from rag.states import GraphState


@pytest.mark.asyncio
async def test_handle_distress_performs_inline_retrieval_when_empty():
    mock_retrieve = AsyncMock(
        return_value={"documents": [{"title": "Teachings", "text": "Be serene"}]}
    )
    mock_rerank = AsyncMock(
        return_value={"reranked_docs": [{"title": "Teachings", "text": "Be serene"}]}
    )
    mock_grade = AsyncMock(
        return_value={"relevant_docs": [{"title": "Teachings", "text": "Be serene"}]}
    )
    mock_ollama = AsyncMock()
    mock_ollama.generate.return_value = "Compassionate response"
    state = GraphState(question="I feel so anxious", chat_history=[], relevant_docs=[])

    with (
        patch("rag.nodes.retrieval.retrieve_documents", mock_retrieve),
        patch("rag.nodes.reranking.rerank_documents", mock_rerank),
        patch("rag.nodes.reranking.grade_documents", mock_grade),
        patch("rag.nodes.intent._services._ollama", mock_ollama),
        patch("rag.nodes.intent._services._serene_mind", None),
    ):
        await handle_distress(state, config={})
        mock_retrieve.assert_called_once()
        mock_rerank.assert_called_once()
        mock_grade.assert_called_once()
        assert mock_ollama.generate.call_count == 1
