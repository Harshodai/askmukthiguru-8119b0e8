...
async def test_pdf_ingestion_routing():
    ...
    with (
        patch("services.http_client_pool.get_client", new=AsyncMock(return_value=mock_client)),
        patch("pypdf.PdfReader", return_value=mock_doc),
        patch.object(
            pipeline._auditor,
            "run",
            new=AsyncMock(return_value=MagicMock(passed=True, score=85, reasons=[])),
        ),
    ):
...
async def test_web_page_scraping_routing():
    ...
    with (
        patch("ingest.web_scraper.fetch_web_article_jina", new=AsyncMock(return_value=None)),
