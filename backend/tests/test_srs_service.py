...
async def test_review_card_sm2_algorithm():
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.execute = MagicMock(return_value=mock_select_resp)
    mock_supabase.table.return_value.update.return_value.eq.return_value.execute = MagicMock(return_value=mock_update_resp)

    service = SRSService(mock_supabase)
    
    # 1. Review with a rating of 5 (perfect response)
    res = await service.review_card("card-1", 5)
    assert res is not None
    assert res["repetitions"] == 1
    assert res["interval_days"] == 1
    
    # Verify the parameters passed to supabase update
