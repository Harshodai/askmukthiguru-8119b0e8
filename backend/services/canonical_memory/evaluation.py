...
class MemoryEvaluator:
    ...
    def evaluate_memory_health(self, user_id: str) -> EvalSuite:
        ...
        results.append(result)
        stale_cutoff = (dt.datetime.now(dt.UTC) - dt.timedelta(days=90)).isoformat()
        stale = [
            m for m in active if (m.get("last_used_at") or m.get("created_at", "")) < stale_cutoff
        ]
        result = EvalResult(
            test_name="freshness",
    ...
    def evaluate_retrieval(self, user_id: str, query: str, expected_facts: list[str]) -> EvalSuite:
        ...
        if not self.retriever:
            suite.results = [
                EvalResult("retriever_available", EvalVerdict.SKIP, 0.0, "No retriever")
            ]
            return suite
        ...
        suite.results = [
            EvalResult(
                "retrieval_recall",
                EvalVerdict.PASS if recall >= 0.9 else EvalVerdict.FAIL,
                recall,
                f"{hits}/{len(expected_facts)} expected found",
            ),
            EvalResult(
                "retrieval_count",
                EvalVerdict.PASS if len(results_found) <= 20 else EvalVerdict.FAIL,
                1.0 if len(results_found) <= 20 else 0.0,
                f"{len(results_found)} results",
            ),
        ]
    ...
    def run_full_eval(self, user_id: str, queries: list[dict[str, Any]] = None) -> dict[str, Any]:
        ...
        return {
            ...
            "health_details": {
                "tests": [(r.test_name, r.verdict.value, r.score) for r in health.results]
            },
