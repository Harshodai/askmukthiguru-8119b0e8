...
class RerankerService:
    ...
    def _ensure_model(self) -> None:
        ...
        with self._lock:
            ...
            if not settings.use_flashrank:
                logger.info("FlashRank is explicitly disabled in settings. Loading sentence-transformers fallback reranker.")
                self._load_fallback()
                return
            ...
            try:
                ...
                if model_name == "auto":
                    ...
                    logger.info(f"Auto-tuning reranker: system={system_platform}, machine={machine}")
                    
                    if system_platform == "Darwin" and ("arm" in machine.lower() or "m1" in machine.lower() or "m2" in machine.lower() or "m3" in machine.lower()):
                        # Apple Silicon (macOS M1/M2/M3/M4) -> use multilingual model for Indian language transcript support
                        model_name = "ms-marco-MultiBERT-L-12"
                        logger.info("Selected 'ms-marco-MultiBERT-L-12' for Apple Silicon to support rich multilingual spiritual wisdom.")
                    else:
                        # Default to lighter model in constrained environments
                        model_name = "ms-marco-MiniLM-L-6-v2"
                        logger.info("Selected light 'ms-marco-MiniLM-L-6-v2' for resource-constrained or generic environments.")
                ...
                logger.info(f"FlashRank successfully initialized in {duration:.4f}s using {model_name}")
                self._is_fallback = False
    ...
    def _load_fallback(self) -> None:
        ...
        try:
            ...
            logger.critical(f"❌ CRITICAL FAILURE: Could not load fallback reranker: {e}", exc_info=True)
            raise e
    ...
    def _rerank_sync(
        ...
        if not self._is_fallback and self._ranker is not None:
            try:
                ...
                for result in results:
                    ...
                    orig_doc["rerank_raw_logit"] = score # FlashRank does not expose logits directly, so use score
                ...
                if filtered_count > 0:
                    logger.info(f"FlashRank threshold {effective_min_score}: filtered {filtered_count} docs below threshold")

                return above_threshold[:top_k]

            except Exception as e:
                logger.error(f"❌ FlashRank rerank execution failed, falling back to CrossEncoder: {e}", exc_info=True)
        ...
        filtered_count = len(reranked_docs) - len(above_threshold)
        if filtered_count > 0:
            logger.info(f"Fallback CrossEncoder threshold {effective_min_score}: filtered {filtered_count} docs below threshold")
