...
class QdrantFilterBuilder:
    ...
    def build_source_url_filter(source_url: str) -> Filter:
        ...
        return Filter(must=[FieldCondition(key="source_url", match=MatchValue(value=source_url))])
    ...
    def build_tags_filter(tags: list[str] | str) -> Filter:
        ...
        if len(tag_values) == 1:
            return Filter(must=[FieldCondition(key="tags", match=MatchValue(value=tag_values[0]))])
        return Filter(must=[FieldCondition(key="tags", match=MatchAny(any=tag_values))])

    @staticmethod
    def build_title_filter(title: str) -> Filter:
        """Return a Qdrant filter matching an exact title (useful for scoped retrieval)."""
        return Filter(must=[FieldCondition(key="title", match=MatchValue(value=title))])
