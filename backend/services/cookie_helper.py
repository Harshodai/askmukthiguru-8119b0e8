...
def ensure_cookies_file(force_refresh: bool = False) -> Optional[str]:
    ...
    if (
        ...
        and os.path.getsize(COOKIES_PATH) > 1000
    ):
        return COOKIES_PATH

    logger.info(
        f"Generating/Refreshing cookies.txt (force_refresh={force_refresh})..."
    )
    ...
    yt_dlp_path = (
        "/Users/harshodaikolluru/Public/askmukthiguru-8119b0e8/.venv_host/bin/yt-dlp"
    )
    ...
    if os.path.exists(COOKIES_PATH) and os.path.getsize(COOKIES_PATH) > 1000:
        logger.warning(
            "Using existing cookies.txt file despite generation failure"
        )
        return COOKIES_PATH
