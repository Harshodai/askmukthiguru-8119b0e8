...
async def generate_skills(atoms_text: str, existing_skills: list[dict]) -> list[dict]:
    ...
    existing_summary = (
        "\n".join(
            f"- {s.get('name', '?')}: {s.get('description', '')[:120]}" for s in existing_skills
        )
        if existing_skills
        else "None yet"
    )
...
async def save_skills(
    supabase_client: Any, user_id: str, tenant_id: str, skills: list[dict]
) -> int:
    saved = 0
    for sk in skills:
        try:
            existing = (
                await supabase_client.table("user_skills")
                .select("id, practice_count, proficiency")
                .eq("user_id", user_id)
...
async def get_skills(
    ...
    try:
        ...
        return [
            {
                "name": "Mindfulness",
                "description": "Basic awareness practice",
                "proficiency": 0.3,
                "practice_count": 0,
            }
        ]
