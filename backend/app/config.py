...
class Settings(BaseSettings):
    ...
    semantic_cache_similarity: float = 0.90  # Cosine similarity threshold for cache hit
...
def _init_api_key_rotator():
    ...
    # Extract keys lists
    def _parse_keys(env_val):
        return [k.strip() for k in (env_val or "").split(",") if k.strip()]

    nim_keys = _parse_keys(os.environ.get(NIM_API_KEY_ENV) or getattr(settings, "nim_api_key", ""))
    openrouter_keys = _parse_keys(os.environ.get(OPENROUTER_API_KEY_ENV) or getattr(settings, "openrouter_api_key", ""))
    ...
    async def patched_send(self, request, *args, **kwargs):
        ...
        if response.status_code in (429, 403):
            ...
            if auth_hdr and auth_hdr.startswith("Bearer "):
                ...
                used_key = sub_hdr
                hdr_name = "api-subscription-key"

            if used_key:
                # Find matching provider
                matched_provider = None
                for provider, lst in key_lists.items():
                    if used_key in lst:
                        matched_provider = provider
                        break

                if matched_provider:
                    lst = key_lists[matched_provider]
                    if len(lst) > 1:
                        try:
                            curr_idx = lst.index(used_key)
                            next_idx = (curr_idx + 1) % len(lst)
                            ...
                            if matched_provider == "nim":
                                settings.nim_api_key = new_key
                            elif matched_provider == "openrouter":
                                settings.openrouter_api_key = new_key
                            elif matched_provider == "sarvam":
                                settings.sarvam_api_key = new_key
                                os.environ[SARVAM_API_KEY_ENV] = new_key
                            elif matched_provider == "krutrim":
                                settings.krutrim_api_key = new_key

                            # Close 429 response to release connection
                            await response.aclose()

                            # Recursively call patched_send (will try new key)
                            return await patched_send(self, request, *args, **kwargs)
                        except ValueError:
                            pass
        return response

    httpx.AsyncClient.send = patched_send
    logger.info("Transparent HTTPX API Key Rotator successfully initialized.")

try:
    _init_api_key_rotator()
except Exception as e:
    import logging
    logging.getLogger("app.config.rotator").error(f"Failed to initialize KeyRotator: {e}")
