...
TIPS_CACHE_KEY = (
    "teachings:tips:v2"  # v2: reject malformed topic labels + strip RAPTOR/Source headers
)
...
async def regenerate_wisdom_tips(
    ...
    logger.info(
        "Wisdom tips regenerated by admin %s (%d tips)",
        sanitize_log_input(str(user.get("id", "unknown"))[:8]),
        len(payload["tips"]),
    )
    return payload
