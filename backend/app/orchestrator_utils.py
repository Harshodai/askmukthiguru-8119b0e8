...
def _claim_subject(claim: str) -> str:
    ...
    Used to deduplicate retrieved memories that cover the same topic, keeping
    the highest-scoring representative.
    """
    text = (claim or "").lower().strip()
    # Drop leading "the seeker", "seeker", "user", or "i" and any following
    # auxiliary or verb to normalize first-person vs third-person phrasing and
    # focus on the topic.
    text = re.sub(
    ...
    # Keep the first noun-like window (up to 6 words) as a stable subject key.
    words = text.split()
...
async def prepare_user_memory(
    ...
    if settings.feature_memory_enabled and getattr(container, "memory_service", None):
        try:
            ...
            if semantic_m:
                # Score by confidence × decay × similarity, top-5, dedupe by subject.
                scored = []
                seen_subjects: set[str] = set()
                for m in sorted(semantic_m, key=lambda x: float(x.get("combined_score") or 0.0), reverse=True):
                    claim = (m.get("claim") or m.get("content") or "").strip()
