...
print(
    f"late_chunking={settings.reingest_late_chunking} backend={settings.embedding_backend}",
    flush=True,
)
...
print(
    "PILOT DONE " + json.dumps({k: v for k, v in res.items() if not isinstance(v, (list, dict))}),
    flush=True,
)
