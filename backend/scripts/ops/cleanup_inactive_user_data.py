...
def cleanup_redis_keys(dry_run: bool = False) -> int:
    ...
    try:
        ...
        keys = r.keys("session:*") + r.keys("ephemeral:*")
        expired_count = 0
        for k in keys:
            ttl = r.ttl(k)
            if ttl == -1 or ttl < 0:  # No TTL set or expired key
                expired_count += 1
                if not dry_run:
                    r.delete(k)
        print(f"✓ Redis cleanup: scanned {len(keys)} keys, purged {expired_count} stale keys.")
        ...
        for col in collections:
            ...
            results, _ = client.scroll(
                ...
                scroll_filter=Filter(
                    must=[FieldCondition(key="updated_at", range={"lt": cutoff})]
                ),
                limit=100,
                with_payload=True,
                with_vectors=False,
...
def main() -> None:
    ...
    print(f"\nCleanup complete in {elapsed:.2f}s. Total purged: Redis={r_count}, Qdrant={q_count}")
