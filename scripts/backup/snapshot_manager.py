...
def get_supabase_container():
    ...
    try:
        cmd = [
            "docker", "ps", "-a",
            "--filter", "ancestor=public.ecr.aws/supabase/postgres:17.6.1.106",
            "--format", "{{.Names}}"
        ]
...
def backup_qdrant(collection_name="spiritual_wisdom"):
    ...
    try:
        ...
        print(f"  [✅] Qdrant backup saved: {QDRANT_BACKUP_PATH} ({os.path.getsize(QDRANT_BACKUP_PATH) / 1024 / 1024:.2f} MB)")
        return True
    except Exception as e:
...
def restore_qdrant(collection_name="spiritual_wisdom"):
    ...
    try:
        ...
        parts.append(b'Content-Disposition: form-data; name="snapshot"; filename="spiritual_wisdom.snapshot"')
        ...
        upload_url = f"{QDRANT_HOST_URL}/collections/{collection_name}/snapshots/upload?priority=snapshot"
        ...
        with urllib.request.urlopen(req) as res:
            ...
            if resp_data.get("status") == "ok":
                print(f"  [✅] Qdrant collection '{collection_name}' successfully restored from snapshot!")
                return True
            else:
...
def backup_neo4j():
    ...
    cmd = [
        ...
        "cypher-shell", "-u", "neo4j", "-p", NEO4J_PASS, cypher_query
    ]
    ...
    try:
        ...
        print(f"  [✅] Neo4j graph backup saved: {NEO4J_BACKUP_PATH} ({len(cypher_text.splitlines())} cypher lines)")
        return True
...
def restore_neo4j():
    ...
    try:
        ...
        result = subprocess.run(cmd, input=cypher_content, capture_output=True, text=True, check=True)
...
def backup_supabase():
    ...
    try:
        ...
        print(f"  [✅] Supabase backup saved: {SUPABASE_BACKUP_PATH} ({os.path.getsize(SUPABASE_BACKUP_PATH)/1024:.2f} KB)")
        return True
    except subprocess.CalledProcessError as e:
...
def restore_supabase():
    ...
    with open(SUPABASE_BACKUP_PATH, "r", encoding="utf-8") as f:
