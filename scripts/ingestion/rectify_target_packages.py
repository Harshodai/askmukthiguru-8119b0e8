...
def rectify_package(pkg_dir: Path, dry_run: bool = False) -> dict:
    ...
    if not seg_file.is_file():
        return {"video_id": vid, "status": "skipped_no_segments"}
    ...
    for s in segments:
        ...
        # Apply updated corrections with ledger from the raw source
    ...
    # Assemble paragraphs for transcript.md
    ...
    # Read existing transcript header
    header_lines = []
    if t_file.is_file():
        old_t = t_file.read_text(encoding="utf-8")
        if "## Transcript" in old_t:
            header_lines = old_t.split("## Transcript", 1)[0].strip().split("\n")
        elif old_t.startswith("---"):
            parts = old_t.split("---", 2)
            if len(parts) == 3:
                header_lines = ("---" + parts[1] + "---").split("\n")
    ...
    if not header_lines:
        header_lines = [
            ...
            "**Quality State:** needs_review",
            "**Quality Score:** 0.70",
            "**Pipeline Version:** 2.0.0",
        ]

    # Update Transcript Hash in header if present
    ...
    if not dry_run:
        ...
        t_file.write_text(new_transcript_md)
        ...
        # Now recompute artifact hashes for manifest
...
if __name__ == "__main__":
    ...
    if args.videos:
        ...
        target_vids = [
            "ACvOem_B-Ek", "CrZuPkgwA6Q", "DBUJH5f6rjU", "aDQhPZUnDqA", "avCLyAi9DeY",
            "-vboUHsZ2KM", "5qA5ibrq9NU", "s-In3a1CAoI", "1WgqAO8AmsY", "4ytgjpow498",
            "dR7olu353VY", "omDPxlDqDs0", "susUKIwVD34", "vLCsNfcbWqs", "vZGe4g66rw4"
        ]
    ...
    for v in target_vids:
        p = corpus_root / v
        if p.is_dir():
            res = rectify_package(p, dry_run=False)
    ...
    print(f"\nDone: {rectified_count} packages processed, {total_ledger} total ledger corrections recorded.")
