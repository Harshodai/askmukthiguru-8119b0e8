...
Output ONLY the JSON array of NEW sections. No explanations, no summaries."""
    ...
    for attempt in range(max_retries + 1):
        ...
        response, finish_reason = llm_completion(model=model, prompt=full_prompt, return_finish_reason=True)
        if finish_reason in ['finished', 'max_output_reached']:
            ...
            print(f'  generate_toc_continue: unexpected finish reason "{finish_reason}" on attempt {attempt+1}')
...
Output ONLY the JSON array. No explanations, no summaries, no markdown."""
    ...
    for attempt in range(max_retries + 1):
        ...
        if finish_reason in ['finished', 'max_output_reached']:
            ...
            print(f'  generate_toc_init: unexpected finish reason "{finish_reason}" on attempt {attempt+1}')
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.9 and len(incorrect_results) > 0:
        ...
        if mode == 'process_toc_with_page_numbers':
            ...
            logger.info('WARNING: All TOC extraction modes failed. Creating fallback flat structure.')
            print('WARNING: All TOC extraction modes failed. Creating fallback flat structure from page groups.')
