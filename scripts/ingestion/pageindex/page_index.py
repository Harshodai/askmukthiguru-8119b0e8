...
def validate_toc_items(items):
    """Validate that TOC items have the required structure fields.
    Returns a filtered list of only valid items, or empty list if none are valid.
    """
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        # Must have at least 'title' and either 'physical_index' or 'structure'
        if "title" not in item and "structure" not in item:
            continue
        # Skip items that look like key-value summaries (wrong format)
        if "key" in item and "value" in item and "title" not in item:
            continue
        valid.append(item)
    return valid
...
async def check_title_appearance(item, page_list, start_index=1, model=None):
    ...
    CRITICAL INSTRUCTION: You MUST output ONLY valid JSON format starting with {{ and ending with }}. Do NOT output summaries, explanations, or conversational text."""
...
def generate_toc_continue(toc_content, part, model=None, max_retries=1):
    print("start generate_toc_continue")
    prompt = """
You are an expert in extracting hierarchical tree structure from documents.
You are given the tree structure extracted from previous parts, and new text from the current part.
Your task is to extract ONLY the NEW sections from the current part text.

Rules:
- "structure" = the numeric hierarchy index continuing from the previous structure (e.g. "3", "3.1", "4")
- "title" = the exact section title from the text (fix spacing only)
- "physical_index" = the <physical_index_X> tag where the section starts

The text uses <physical_index_X> tags to mark page boundaries.

Example output (continuing from previous structure ending at "2.3"):
[
    {"structure": "3", "title": "Next Chapter", "physical_index": "<physical_index_42>"},
    {"structure": "3.1", "title": "First Subsection", "physical_index": "<physical_index_43>"}
]

IMPORTANT: Each item MUST have exactly these three keys: "structure", "title", "physical_index".
Output ONLY the JSON array of NEW sections. No explanations, no summaries."""
    ...
    for attempt in range(max_retries + 1):
        full_prompt = (
            ...
            + '\n\nOutput ONLY a JSON array with keys "structure", "title", "physical_index":'
        )
        response, finish_reason = llm_completion(
            model=model, prompt=full_prompt, return_finish_reason=True
        )
        if finish_reason in ["finished", "max_output_reached"]:
            result = extract_json(response)
            if not isinstance(result, list):
                result = []
            valid_items = validate_toc_items(result)
            if valid_items:
                return valid_items
            print(f"  generate_toc_continue: invalid format on attempt {attempt+1}, retrying...")
        elif finish_reason == "error":
            print(f"  generate_toc_continue: LLM error on attempt {attempt+1}, retrying...")
        else:
            print(
                f'  generate_toc_continue: unexpected finish reason "{finish_reason}" on attempt {attempt+1}'
            )

    print("  generate_toc_continue: all attempts failed, returning empty list")
    return []


### add verify completeness
def generate_toc_init(part, model=None, max_retries=1):
    print("start generate_toc_init")
    prompt = """
You are an expert in extracting hierarchical tree structure from documents.

Task: Generate a JSON array representing the table of contents / section structure of the given document text.
...
[
    ...
    {"structure": "2", "title": "Main Content", "physical_index": "<physical_index_5>"}
]

IMPORTANT: Each item MUST have exactly these three keys: "structure", "title", "physical_index".
Output ONLY the JSON array. No explanations, no summaries, no markdown."""
    ...
    for attempt in range(max_retries + 1):
        ...
        if finish_reason in ["finished", "max_output_reached"]:
            ...
            if valid_items:
                return valid_items
            # If no valid items, retry with even more explicit prompt
            print(f"  generate_toc_init: invalid format on attempt {attempt+1}, retrying...")
        elif finish_reason == "error":
            print(f"  generate_toc_init: LLM error on attempt {attempt+1}, retrying...")
        else:
            print(
                f'  generate_toc_init: unexpected finish reason "{finish_reason}" on attempt {attempt+1}'
            )

    # Final fallback: return empty list (caller will handle)
    print("  generate_toc_init: all attempts failed, returning empty list")
    return []
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    # Validate items have the right structure before converting
    toc_with_page_number = validate_toc_items(toc_with_page_number)
    if not toc_with_page_number:
        logger.info("WARNING: generate_toc returned no valid items, all items were malformed")
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Handle empty list: nothing to verify
    if not list_result:
        print("verify_toc: empty list_result, returning accuracy=0")
        return 0, []
    ...
    if not indexed_sample_list:
        print("verify_toc: no items with valid physical_index to check")
        return 0, []

    # Run checks concurrently
    tasks = [
        check_title_appearance(item, page_list, start_index, model) for item in indexed_sample_list
    ]
...
def _create_fallback_structure(page_list, start_index=1, pages_per_section=10):
    """Create a minimal flat TOC structure when LLM-based extraction fails entirely.
    Groups pages into sections of `pages_per_section` pages each.
    """
    fallback = []
    total_pages = len(page_list)
    section_num = 1
    for page_start in range(0, total_pages, pages_per_section):
        page_idx = start_index + page_start
        fallback.append(
            {
                "structure": str(section_num),
                "title": f"Section {section_num} (Pages {page_idx}-{min(page_idx + pages_per_section - 1, start_index + total_pages - 1)})",
...
async def meta_processor(
    ...
    if accuracy == 1.0 and len(incorrect_results) == 0:
        return toc_with_page_number
    if accuracy > 0.9 and len(incorrect_results) > 0:
        ...
        if mode == "process_toc_with_page_numbers":
            ...
            # Final fallback: create a minimal flat structure from the raw pages
            # This prevents a hard crash when the LLM consistently returns malformed output
            logger.info(
                "WARNING: All TOC extraction modes failed. Creating fallback flat structure."
            )
            print(
                "WARNING: All TOC extraction modes failed. Creating fallback flat structure from page groups."
            )
            fallback = _create_fallback_structure(page_list, start_index)
            return fallback
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    tasks = [
        process_large_node_recursively(node, page_list, opt, logger=logger) for node in toc_tree
    ]
    await asyncio.gather(*tasks)
...
def page_index_main(doc, opt=None):
    ...
    logger.info({"total_text_units": sum([page[1] for page in page_list])})
