...
unexpected=0 if set(audit.get('issue_counts',{})) <= {'incomplete_package'} else audit.get('issue_total',0)
...
md2=['# Sanskrit Correction Application Report','', '**Application completed from the validated patch after a reversible pre-application backup.** The corpus was intentionally modified only in the 25 affected complete packages.','','| Metric | Result |','|---|---:|',f"| Validated candidate rows requested | {apply['requested_rows']} |",f"| Literal replacements applied | {apply['actual_replacements']} |",f"| Affected packages | {len(changed)} |",f"| New canonical correction-ledger entries | {ledger_added} |",f"| Transcript-only derivative replacements | {apply['actual_replacements']-ledger_added} |",f"| Manual-review rows left unchanged | {len(manual)} |",f"| Complete packages after audit | {audit['complete_packages']} |",f"| Incomplete packages | {audit['incomplete_packages']} |",f"| Unexpected integrity issues in complete packages | {unexpected} |",'', '## Integrity result','', 'The post-application audit reports no unexpected issues in the 657 complete packages. The only remaining audit findings are the pre-existing 88 incomplete package directories, represented as 440 missing-artifact records. All modified packages have updated canonical segments, transcripts, correction ledgers, quality-report counts, artifact-manifest hashes, and legacy manifest hashes.', '', '## Reversal','', 'A complete pre-application copy and SHA-256 backup manifest is stored under `confirmed_patch/backup_before_apply`. Reversal should restore those package directories as a unit; do not selectively reverse individual manifest files.']
