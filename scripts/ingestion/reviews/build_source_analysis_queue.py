...
for pkg in sorted(p for p in corpus.iterdir() if p.is_dir()):
    ...
    m=re.search(r'\*\*URL:\*\*\s*(https?://(?:www\.)?youtube\.com/watch\?v=[^\s`]+)',t)
    if m:
        url=m.group(1).rstrip('.,)')
    else:
        m=re.search(r'https?://(?:www\.)?youtube\.com/watch\?v=[A-Za-z0-9_-]+',t)
        url=m.group(0).rstrip('.,)') if m else f'https://www.youtube.com/watch?v={pkg.name}'
