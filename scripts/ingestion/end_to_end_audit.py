...
def audit(root,out):
 ...
 for p in pkgs:
  ...
  qd=q if isinstance(q,dict) else {}; ids=[str(s.get('segment_id','')) for s in ss]; by={str(s.get('segment_id')):s for s in ss}; prev=-1; duration=float(qd.get('duration_seconds') or 0)
  ...
  last_corr={str(e2.get('segment_id','')):j for j,e2 in enumerate(es)}
  for i,e in enumerate(es):
   ...
   if corr is not None and last_corr.get(sid)==i and corr!=s.get('text'):local.append(('high','ledger_corrected_text_mismatch',str(i)))
  ...
  if isinstance(man,dict):
   ...
   if qd.get('quality_state')!=man.get('final_quality_state'):local.append(('high','quality_state_manifest_mismatch',''))
  ...
  rec.append({'video_id':vid,'package_complete':True,'quality_state':qd.get('quality_state'),'quality_score':qd.get('quality_score'),'segment_count':len(ss),'ledger_count':len(es),'issue_count':len(local),'manifest_hash':man.get('manifest_hash')})
