...
for pattern in "${patterns[@]}"; do
  while IFS= read -r line; do
    # Skip matches in .venv, node_modules, .git
    ...
    --exclude-dir=dist --exclude-dir=dist-ssr --exclude-dir=build \
    --exclude-dir=__pycache__ --exclude-dir=.pytest_cache \
    --exclude-dir=reports --exclude-dir=playwright-report \
    2>/dev/null || true)
done
...
git -C "$ROOT_DIR" log --all --diff-filter=A --pretty=format:"  %h %s" -- '*scratch*' 2>/dev/null || echo "  No scratch files in git history"
...
if [ -f "$ROOT_DIR/.env.example" ]; then
  ...
  if grep -qiE '(sk_live|pk_live|ghp_|AIza)' "$ROOT_DIR/.env.example" 2>/dev/null; then
    echo "  ⚠️  .env.example contains what looks like real secrets!"
  else
