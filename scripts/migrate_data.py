...
Env: MIGRATE_LIMIT=N (default 0 = no limit) processes only the first N unique
sources — used by the orchestrator for a 1-source trial replay before full run.
"""
...
async def migrate():
    ...
    try:
        ...
        # MIGRATE_LIMIT: trial-replay knob; default 0 means no limit.
        try:
            migrate_limit = int(os.environ.get("MIGRATE_LIMIT", "0"))
        except ValueError:
            migrate_limit = 0
        if migrate_limit > 0:
            limited_sources = dict(list(sources.items())[:migrate_limit])
            skipped = len(sources) - len(limited_sources)
            sources = limited_sources
            logger.info(
                f"Trial mode: limiting to {migrate_limit} source(s) "
                f"per MIGRATE_LIMIT. Skipped the remaining {skipped} sources."
            )
